...
# Last modified (work PC)
# 2/8/2016 changed restore kill connections option for replace restore
# 4/3/2015 added call to CheckDBFolder in Ok_To_Restore
# 4/2/2015 more fixes in Ok to restore, fixes in file data types  L,D,F,S, added CheckDBFolders
# 3/30/2015 did fixes in suffix in in restore
# 3/16/2015 fixed issue with case in data and folder names, added upper (self.datafolder[0].upper() == x.upper()) in ok_to_restore
# 3/12/2015 cleaned code
# 3/9/2015  added and tested BuiltTlogSQL
# 3/4/2015  did fix in GetDatabaseInfo for when database does not exist
# 3/4/2015  modified GetDiskPace for issue when no db present, return empty dict
# 2/19/2015 did fix in RestoreDatabases in file naming (moved up i = i + 1
# 1/29/2015 added killconnections sql to restore script, no need to call KillConnections on restores
# 1/14/2015 more fixes (all values in gb), changed GetDBInfo (part of Ok_to_restore fix)
# 1/13/2015 fixed Ok_to_restore
# 1/?/2015 changed KillConnections
# 1/5/2015 changed SqlExecute to use subprocess.Popen
# 12/30/2014 added DatedString
# 12/24/2014 added DeleteOlderFiles
# 12/22/2014 moved killconnections out of SQLSMO class and made function for synclogins
# 12/13/2014 changed call to GetBackupInfo
# 12/9/2014 Added GIT
# 12/5/2014
...
class SQLSMO:
    ...
    def Ok_to_restore(self):
        ...
        if CheckDBFolders(self.conn, self.datafolder, self.logfolder) is False:
            return False
        ...
        for x in dict_freespace.keys():                      # x is the drive letter
            if self.datafolder[0].upper() == x.upper():      # if letter of datafolder matches drive, add to data value
                data_disk += float(dict_freespace[x])
            if self.logfolder[0].upper() == x.upper():       # if letter of logfolder matches drive, add to log value
    ...
    def RestoreDatabase(self):
        ...
        self.GetBackupInfo()
        if self.dictfiles is False:
            print('Invalid file names from backup')
            return []

       # This is for the option of having dated string in the database files, like db_data_09122015.mdf
        t = ''
        ...
        if self.restore_options['replace']:
            sd = "ALTER DATABASE [" + self.currdb + "] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE"
            self.sqlrestore = sd + " ; " + s
        else:
            self.sqlrestore = + s
    ...
    # NEW 7/16/2015
    # def BuildTlogSQL(dbname, sourcedb, bkfolder, recovery = 'NORECOVERY', BACKUP_MASK = '_backup_20*.BAK', LOG_MASK = '_backup_20*.TRN'):
    # def RestoreLogs(self):
        # s = BuildTlogSQL(dbname, sourcedb, bkfolder, recovery = 'NORECOVERY', BACKUP_MASK = '_backup_20*.BAK', LOG_MASK = '_backup_20*.TRN'):
        # s = BuildTlogSQL(dbname, sourcedb, bkfolder, recovery = 'NORECOVERY', BACKUP_MASK = '_backup_20*.BAK', LOG_MASK = '_backup_20*.TRN'):
...
if __name__ == '__main__':
    ...
    print('Check the given database folders:')
    r = CheckDBFolders(CONNECTION, DATAFOLDER, LOGFOLDER)
    if r:
        print('Folder locations are good')
    print('Mangling folder locations and retesting')
    DATAFOLDER2 = r'D:\SQL2014\DATABASES2'
    LOGFOLDER2 = r'D:\SQL2014\DATABASES2'
    r = CheckDBFolders(CONNECTION, DATAFOLDER2, LOGFOLDER2)
